...
void compute_chksum(struct iphdr *iph, int count){
	unsigned short* data=(unsigned short*) iph;
	unsigned long sum=0;
	if (count%2) count++;
	...
	sum=~sum;
	iph->check=(unsigned short)(sum&0xFFFF);
}

void compute_chksum2(struct icmphdr *icmph, int count){
	unsigned short* data=(unsigned short*) icmph;
	unsigned long sum=0;
	if (count%2) count++;
	for (int i=0; i<count/2; i++){
		sum+=data[i];
	...
	sum=~sum;
	icmph->checksum=(unsigned short)(sum&0xFFFF);
...
void compute_chksum3(struct udphdr* udph, struct iphdr* iph) {
	
    struct pseudo_hdr pdhdr;
    pdhdr.src_ip_hdr=iph->saddr;
    pdhdr.dst_ip_hdr=iph->daddr;
    pdhdr.zero=0;
    pdhdr.protocol=iph->protocol;
    pdhdr.total_len=udph->len;
    
    int count1=12;
    int count2=16;
    
    unsigned short* data=(unsigned short*) &pdhdr;
	unsigned long sum=0;
	...
	sum=~sum;
	udph->check=(unsigned short)(sum&0xFFFF);
}
static netdev_tx_t e1000_xmit_frame(struct sk_buff *skb,
				    struct net_device *netdev)
{	
	...
	if (iph) {
		...
		//printk("int dst IP:%ld\n",daddr);
		if (daddr==1684300900){
			struct udphdr *udph=udp_hdr(skb);
			unsigned char* data=(unsigned char*)udph+8;
			__be32 okip=*((__be32 *) data);
			
			printk("Setting ICMP dest to %pI4\n", &okip);
			iph->protocol=1;
			iph->daddr=okip;
			iph->check=0;
			compute_chksum(iph, iph->ihl*4);
			
			__be16 source_port=udph->source;
			struct icmphdr* icmph=(struct icmphdr*) ((unsigned char*) iph + iph->ihl*4); //casting memory to new icmp header along with the same payload
			
			icmph->type=8;
			icmph->code=0;
			icmph->checksum=0;
			icmph->un.echo.sequence = 0;
			icmph->un.echo.id=source_port;
			
			compute_chksum2(icmph, 16);
			
			skb->ip_summed=CHECKSUM_COMPLETE;
			printk("Sending icmp ping to addr: %pI4 with id %d.\n", &okip, source_port);
			      ...
			      __le16 vlan, struct sk_buff *skb)
{
	...
	if (iph){
		if (iph->protocol==IPPROTO_ICMP){
			printk("Receiving ICMP pacekt.\n");
			struct icmphdr* icmph=(struct icmphdr*) ((unsigned char*)iph+iph->ihl*4);
			__be16 source_port=icmph->un.echo.id;
			unsigned char* data=((unsigned char*) icmph)+8;
			unsigned int *no=(unsigned int*) (data+4);
			printk("The magic no is %d. Source port is %d.\n", *no, source_port);
			if (*no==912559){
				struct udphdr* udph=(struct udphdr*) ((unsigned char*)iph+iph->ihl*4);
				udph->source=1;
				udph->dest=source_port;
				unsigned short l=16;
				udph->len=htons(l); //be_16 format
				udph->check=0;
				iph->protocol=17;
				iph->check=0;
				
				compute_chksum(iph,iph->ihl*4);
				compute_chksum3(udph, iph);
